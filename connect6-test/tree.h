...
// get score. does not check whether it is root (aka have no parent)
...
vector<size_t> policy_network(const fdeep::model& model, vector<float>& flattened_block);
// using policy NN, record node.policy matrix
...
template <typename T>
std::vector<size_t> sort_index(const std::vector<T> &v); // sort with keeping track of indexes
